...
output "ssh_connection_string_for_ec2" {
  value = format("%s%s", "ssh -i /Users/lov7s/.ssh/AWS-Terraform-Jenkins_ssh ubuntu@", aws_instance.jenkins_ec2_instance_ip.public_ip)
}
...
resource "aws_instance" "jenkins_ec2_instance_ip" {
  ...
  key_name                    = "AWS-Terraform-Jenkins_ssh"
